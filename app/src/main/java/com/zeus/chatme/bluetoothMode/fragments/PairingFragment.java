...
import com.zeus.chatme.bluetoothMode.gui.ButtonSearch;
import com.zeus.chatme.bluetoothMode.gui.CustomAnimator;
import com.zeus.chatme.bluetoothMode.gui.GuiTools;
import com.zeus.chatme.bluetoothMode.gui.PeerListAdapter;
import com.zeus.chatme.bluetoothMode.gui.RequestDialog;
import com.zeus.chatme.bluetoothMode.tools.Tools;
...
public class PairingFragment extends Fragment {
    ...
    private BluetoothMainActivity.Callback communicatorCallback;
    private CustomAnimator animator = new CustomAnimator();
    private Peer connectingPeer;
    protected Global global;
    protected BluetoothMainActivity activity;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        communicatorCallback = new BluetoothMainActivity.Callback() {
            @Override
            ...
            public void onConnectionSuccess(Peer peer, int source) {
                ...
                activity.setFragment(BluetoothMainActivity.CONVERSATION_FRAGMENT);
            }
    ...
    public void onStart() {
        ...
        if (!Tools.hasPermissions(activity, BluetoothMainActivity.REQUIRED_PERMISSIONS)) {
            startSearch();
        }
    ...
    protected void startSearch() {
        ...
        if (result != BluetoothCommunicator.SUCCESS) {
            ...
            } else if (result != BluetoothMainActivity.NO_PERMISSIONS && result != BluetoothCommunicator.ALREADY_STARTED) {
    ...
    public void disappearLoading(final boolean appearSearchButton, @Nullable CustomAnimator.EndListener responseListener) {
        ...
        if (!isLoadingAnimating) {
            if (loading.getVisibility() != View.GONE) {  // if the object has not already disappeared graphically
                ...
                isLoadingAnimating = true;
                int loadingSizePx = GuiTools.convertDpToPixels(activity, LOADING_SIZE_DP);
                Animator animation = animator.createAnimatorSize(loading, loadingSizePx, loadingSizePx, 1, 1, 200);
                animation.addListener(new Animator.AnimatorListener() {
                    @Override
                    public void onAnimationStart(Animator animation) {
                    }
